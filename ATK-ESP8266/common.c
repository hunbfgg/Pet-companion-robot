...
const u8* wifista_ssid="OPPO R15";			//路由器SSID号
...
PRESTA:		
	...
	{
		LCD_Clear(WHITE);
		if(atk_8266_ip_set("WIFI-AP远端UDP IP设置","UDP模式",(u8*)portnum,"a18qhFfWs3H.iot-as-mqtt.cn-shanghai.aliyuncs.com"))goto PRESTA;	//IP输入
		if(netpro&0X03)sprintf((char*)p,"AT+CIPSTART=1,\"UDP\",\"%s\",%s","a18qhFfWs3H.iot-as-mqtt.cn-shanghai.aliyuncs.com",(u8*)portnum);    //配置目标UDP服务器,及ID号，STA模式下为0
		else sprintf((char*)p,"AT+CIPSTART=0,\"UDP\",\"%s\",%s","a18qhFfWs3H.iot-as-mqtt.cn-shanghai.aliyuncs.com",(u8*)portnum);    //配置目标UDP服务器,及ID号，STA模式下为0
		delay_ms(200);
		LCD_Clear(WHITE);
	...
	{ 
		...
		{   
			...
			}else
			{
				...
				if(key==14){   Show_Str(30,140,200,16,"好的",16,0);	break;} 		//确认连接 
