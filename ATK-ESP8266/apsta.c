...
PRESTA:
	...
	if(netpro&0X02)   //STA UDP
	{
				...
				if(atk_8266_ip_set("WIFI-STA远端UDP IP设置","UDP模式",(u8*)portnum,"a18qhFfWs3H.iot-as-mqtt.cn-shanghai.aliyuncs.com"))goto PRESTA;	//IP输入
				sprintf((char*)p,"AT+CIPSTART=0,\"UDP\",\"%s\",%s","a18qhFfWs3H.iot-as-mqtt.cn-shanghai.aliyuncs.com",(u8*)portnum);    //配置目标UDP服务器,及ID号，STA模式下为0
				LCD_Clear(WHITE);
				Show_Str_Mid(0,30,"ATK-ESP WIFI-STA测试",16,240); 
				Show_Str(30,50,200,16,"正在配置ATK-ESP模块,请稍等...",12,0);
	...
	else     //TCP
	{
		if(netpro&0X01)     //STA TCP Client  
		{
			...
			Show_Str(30,50,200,16,"正在配置ATK-ESP模块,请稍等...",12,0);
			if(atk_8266_ip_set("WIFI-STA 远端IP设置",(u8*)ATK_ESP8266_WORKMODE_TBL[netpro],(u8*)portnum,"a18qhFfWs3H.iot-as-mqtt.cn-shanghai.aliyuncs.com"))goto PRESTA;	//IP输入
			sprintf((char*)p,"AT+CIPSTART=0,\"TCP\",\"%s\",%s","a18qhFfWs3H.iot-as-mqtt.cn-shanghai.aliyuncs.com",(u8*)portnum);    //配置目标TCP服务器,及ID号，STA模式下为0
